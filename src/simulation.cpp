...
sim_t::BaseSimulation(const uint16_t& queue_lim, const uint16_t& service_lim, const float& margin) {
  _queue_limit = queue_lim;
  _margin = margin;
  _service_limit = service_lim;
}

void sim_t::simulate(float simulate_time,
...
void sim_t::add_task(const task_t& task) {
  #ifdef DEBUG
...
const task_t& sim_t::get_task() const {
  #ifdef DEBUG
...
void sim_t::add_client(const client_t& cl) {
  ...
  if(_client_queue.size() < _queue_limit) {
    ...
    #endif
    ++_accept_count;
    return _client_queue.push(cl);
  }

  ++_deny_count;
}

void sim_t::pop_client() {
  #ifdef DEBUG
...
client_t& sim_t::get_client() {
  return _client_queue.front();
}

void sim_t::status() const {
  #ifdef DEBUG
...
const client_t& sim_t::get_service() const {
